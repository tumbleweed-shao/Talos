...
int DiskInfo::ReadPhysicalDriveInNTUsingSmart (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      {
         GETVERSIONINPARAMS_2 GetVersionParams;
         ...
         if (  DeviceIoControl (hPhysicalDriveIOCTL, SMART_GET_VERSION,
                   NULL, 
                   0,
     			   &GetVersionParams, sizeof (GETVERSIONINPARAMS_2),
				   &cbBytesReturned, NULL) )
         {         
	           // Allocate the command buffer
			ULONG CommandSize = sizeof(SENDCMDINPARAMS_2) + IDENTIFY_BUFFER_SIZE;
        	PSENDCMDINPARAMS_2 Command = (PSENDCMDINPARAMS_2) malloc (CommandSize);
...
#define ID_CMD          0xEC            // Returns ID sector for ATA
	        ...
	        if ( DeviceIoControl (hPhysicalDriveIOCTL, 
				                    SMART_RCV_DRIVE_DATA, Command, sizeof(SENDCMDINPARAMS_2),
									Command, CommandSize,
									&BytesReturned, NULL) )

			{
                ...
                USHORT *pIdSector = (USHORT *)
                             (PIDENTIFY_DATA) ((PSENDCMDOUTPARAMS_2) Command) -> bBuffer;
...
int DiskInfo::ReadPhysicalDriveInNTWithAdminRights (void)
{
   int done = FALSE;
   int drive = 0;
   BYTE IdOutCmd [sizeof (SENDCMDOUTPARAMS_2) + IDENTIFY_BUFFER_SIZE - 1];
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      HANDLE hPhysicalDriveIOCTL = 0;
         ...
         {         
			 ...
			 if (VersionParams.bIDEDeviceMap > 0)
			 {
				...
				if ( DoIDENTIFY (hPhysicalDriveIOCTL, 
						   (PSENDCMDINPARAMS_2)&scip, 
						   (PSENDCMDOUTPARAMS_2)&IdOutCmd, 
						   (BYTE) bIDCmd,
						   (BYTE) drive,
						   &cbBytesReturned))
				{
					...
					USHORT *pIdSector = (USHORT *)
								 ((PSENDCMDOUTPARAMS_2) IdOutCmd) -> bBuffer;
				   AddIfNew(pIdSector);	  
				   done = TRUE;
				}
			}
...
typedef enum __STORAGE_QUERY_TYPE {
    PropertyStandardQuery_2 = 0,          // Retrieves the descriptor
    PropertyExistsQuery_2,                // Used to test whether the descriptor is supported
    PropertyMaskQuery_2,                  // Used to retrieve a mask of writeable fields in the descriptor
    PropertyQueryMaxDefined_2     // use to validate the value
} STORAGE_QUERY_TYPE_2, *PSTORAGE_QUERY_TYPE_2;

//
// define some initial property id's
//

typedef enum __STORAGE_PROPERTY_ID {
    StorageDeviceProperty_2 = 0,
    StorageAdapterProperty_2
} STORAGE_PROPERTY_ID_2, *PSTORAGE_PROPERTY_ID_2;
...
typedef struct __STORAGE_PROPERTY_QUERY {

    //
    // ID of the property being retrieved
    //

    STORAGE_PROPERTY_ID_2 PropertyId;

    //
    // Flags indicating the type of query being performed
    //

    STORAGE_QUERY_TYPE_2 QueryType;
...
} STORAGE_PROPERTY_QUERY_2, *PSTORAGE_PROPERTY_QUERY_2;
...
typedef struct __STORAGE_DEVICE_DESCRIPTOR {
...
} STORAGE_DEVICE_DESCRIPTOR_2, *PSTORAGE_DEVICE_DESCRIPTOR_2;
...
int DiskInfo::ReadPhysicalDriveInNTWithZeroRights (void)
{
   ...
   for (drive = 0; drive < MAX_IDE_DRIVES; drive++)
   {
      ...
      if (hPhysicalDriveIOCTL != INVALID_HANDLE_VALUE)
      {
		 STORAGE_PROPERTY_QUERY_2 query;
         DWORD cbBytesReturned = 0;
		 char buffer [10000];

         memset ((void *) & query, 0, sizeof (query));
		 query.PropertyId = StorageDeviceProperty_2;
		 query.QueryType = PropertyStandardQuery_2;
         ...
         if ( DeviceIoControl (hPhysicalDriveIOCTL, IOCTL_STORAGE_QUERY_PROPERTY,
                   ...
                   & cbBytesReturned, NULL) )
         {         
			 STORAGE_DEVICE_DESCRIPTOR_2 * descrip = (STORAGE_DEVICE_DESCRIPTOR_2 *) & buffer;
...
BOOL DiskInfo::DoIDENTIFY (HANDLE hPhysicalDriveIOCTL, PSENDCMDINPARAMS_2 pSCIP,
                 PSENDCMDOUTPARAMS_2 pSCOP, BYTE bIDCmd, BYTE bDriveNum,
                 PDWORD lpcbBytesReturned)
{
   ...
   return ( DeviceIoControl (hPhysicalDriveIOCTL, DFP_RECEIVE_DRIVE_DATA,
               (LPVOID) pSCIP,
               sizeof(SENDCMDINPARAMS_2) - 1,
               (LPVOID) pSCOP,
               sizeof(SENDCMDOUTPARAMS_2) + IDENTIFY_BUFFER_SIZE - 1,
...
#define  SENDIDLENGTH  sizeof (SENDCMDOUTPARAMS_2) + IDENTIFY_BUFFER_SIZE
   ...
   for (controller = 0; controller < 16; controller++)
   {
      ...
      if (hScsiDriveIOCTL != INVALID_HANDLE_VALUE)
      {
         ...
         for (drive = 0; drive < 2; drive++)
         {
            ...
            SENDCMDINPARAMS_2 *pin =
                   (SENDCMDINPARAMS_2 *) (buffer + sizeof (SRB_IO_CONTROL));
            DWORD dummy;
            ...
            if (DeviceIoControl (hScsiDriveIOCTL, IOCTL_SCSI_MINIPORT, 
                                 ...
                                 &dummy, NULL))
            {
               SENDCMDOUTPARAMS_2 *pOut =
                    (SENDCMDOUTPARAMS_2 *) (buffer + sizeof (SRB_IO_CONTROL));
